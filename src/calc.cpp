...
int user_total_holidays(int category, int days, char sport, char culture) {  //выставление счета на проживание
    ...
    int number = 4500;
    int price_sport = 0;		//стоимость аренты спортивного инвентаря (лыжи, сноуборд), абонемент на посещение бассейна
    int price_culture = 0;	//стоимость посещения развлекательных мероприятий (экскурсии, концерты)
    int dop_option = 0;
